import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';
import LanguageDetector from 'i18next-browser-languagedetector';

i18n
  .use(LanguageDetector)
  .use(initReactI18next)
  .init({
    resources: {
      en: {
        translation: {
          "appName": "TASK MANAGER",
          "home": "Home",
          "dashboard": "Dashboard",
          "progress": "BOARD PROGRESS",
          "search": "Search tasks...",
          "createTask": "Create New Task",
          "title": "Title",
          "description": "Description",
          "addTask": "ADD TASK",
          "clear": "Clear", 
          "noTasks": "No tasks here",
          "comments": "Comments",
          "noComments": "No comments yet. Be the first!",
          "commentPlaceholder": "Write a comment...",
          "commentSend": "Send",
          "commentError": "Could not send comment. Try again.",
          "deleteComment": "Delete comment",
          "deleteCommentConfirmTitle": "Delete this comment?",
          "deleteCommentConfirmBody": "This will permanently delete the comment for everyone. This action cannot be undone.",
          "deleteCommentError": "Could not delete comment. Try again.",
          "clearChat": "Clear chat",
          "clearChatConfirmTitle": "Clear chat for you?",
          "clearChatConfirmBody": "This will hide all current messages from your view only. Other participants will still see them, and they will reappear if you reload after they're re-synced. It does not delete anything from the server.",
          "chatCleared": "Chat cleared for you.",
          "you": "You",
          "required": "required",
          "pending": "PENDING",
          "inProgress": "IN PROGRESS",
          "completed": "COMPLETED",
          "all": "ALL",
          "placeholderTitle": "What needs to be done?",
          "placeholderDesc": "Add some details...",
          "viewSource": "View Source on GitHub",
          "syncing": "Synchronizing with DB...",
          "editTask": "Edit Task",
          "taskDetails": "Task Details",
          "saveChanges": "Save Changes",
          "cancel": "Cancel",
          "close": "Close",
          "deleteWarning": "Are you sure you want to delete",
          "deleteAction": "This action is permanent.",
          "deleteTask": "Delete Task",
          "moveTo": "Move to",
          "noDescription": "No description",
          "noDetails": "No additional details provided for this task.",
          "sortNewest": "Newest first",
          "sortOldest": "Oldest first",
          "page": "Page",
          "of": "of",
          "prev": "Prev",
          "next": "Next",
          "firstPage": "First page",
          "lastPage": "Last page",
          "kpiDashboard": "KPI Dashboard",
          "totalTasks": "Total Tasks",
          "completionRate": "Completion Rate",
          "statusDistribution": "Task Breakdown (Status)",
          "recentActivity": "System Activity Log",
          "createdOn": "Created",
          "updated": "Updated",
          "noActivity": "No recent activity.",
          "avgTime": "Avg. Completion Time",
          "days": " days",
          "and": " and ",
          "hours": " hours",
          "lessThanDay": "<1 day",
          "boardHealth": "Board Health",
          "workloadTitle": "Workload by Status",
          "healthExcellent": "EXCELLENT",
          "healthImprovable": "IMPROVABLE",
          "errorTitle": "Connection Issue",
          "errorMessage": "We couldn't reach the server. Please check your connection.",
          "errorLoadingTasks": "Failed to sync tasks",
          "retry": "Retry",
          "taskCreated": "Task created successfully!",
          "taskUpdated": "Task updated successfully!",
          "taskDeleted": "Task removed",
          "clearCompleted": "Clear Completed",
          "completedCleared": "Completed tasks have been removed",
          "clearCompletedWarning": "Are you sure? All COMPLETED tasks will be permanently deleted.",
          "requiredFieldsError": "Title and Description are required",
          "loginSubtitle": "Sign in to manage your tasks",
          "loginEmail": "Email",
          "loginEmailPlaceholder": "you@example.com",
          "loginPassword": "Password",
          "loginPasswordPlaceholder": "Your password",
          "loginButton": "Sign In",
          "loginRequiredFields": "Email and password are required",
          "loginError": "Invalid credentials. Please try again.",
          "accountBlocked": "Your account has been blocked. Please contact an administrator.",
          "togglePassword": "Toggle password visibility",
          "adminPanel": "Admin Panel",
          "adminRole": "Administrator", 
          "userRole": "User",           
          "adminTotalUsers": "Total Users",
          "adminTotalTasks": "Total Tasks",
          "adminInProgress": "In Progress",
          "adminTasksPerUser": "Tasks per User",
          "resourceManagement": "Resource Management",
          "leadTimeByCategory": "Avg. Resolution Time by Category",
          "avgLeadTime": "Avg. Lead Time",
          "overloaded": "Overloaded",
          "healthy": "Healthy",
          "noActiveWork": "No active tasks at the moment.",
          "noLeadTimeData": "No completed tasks to calculate lead time yet.",
          "rangeLast7": "Last 7 days",
          "rangeLast30": "Last 30 days",
          "rangeLast90": "Last 90 days",
          "rangeAll": "All time",
          "adminUserList": "User Management",
          "adminSearchUsers": "Search by name or email...",
          "adminColName": "Name",
          "adminColEmail": "Email",
          "adminColRole": "Role",
          "adminColTotal": "Total",
          "adminColActions": "Actions",
          "adminPromote": "Promote",
          "adminDemote": "Demote",
          "adminPromoted": "User promoted to Admin",
          "adminDemoted": "User demoted to User",
          "adminPromoteConfirm": "Promote {{name}} to ADMIN? They will gain full access.",
          "adminDemoteConfirm": "Demote {{name}} to USER? They will lose admin access.",
          "blockUser": "Block",
          "unblockUser": "Unblock",
          "blockUserConfirm": "Block {{name}}? They will be kicked out on their next request.",
          "unblockUserConfirm": "Unblock {{name}}? They will regain access immediately.",
          "userBlocked": "User blocked successfully.",
          "userUnblocked": "User unblocked successfully.",
          "deleteUser": "Delete user",
          "userDeleted": "User deleted.",
          "deleteUserConfirm": "Delete {{name}}?",
          "deleteUserWarning": "This will permanently delete the account and all their tasks ({{count}}). This action cannot be undone.",
          "noResults": "No results found.",
          "registerSubtitle": "Create your account",
          "registerName": "Name (optional)",
          "registerNamePlaceholder": "Your name",
          "registerConfirmPassword": "Confirm Password",
          "registerConfirmPasswordPlaceholder": "Repeat your password",
          "registerButton": "Create Account",
          "registerLink": "Don't have an account? Register",
          "loginLink": "Already have an account? Sign in",
          "registerPasswordsNoMatch": "Passwords do not match.",
          "registerPasswordTooShort": "Password must be at least 6 characters.",
          "registerEmailTaken": "This email is already registered.",
          "editProfileTitle": "Edit Profile",
          "editProfileName": "Display name",
          "editProfileSave": "Save",
          "editProfileSuccess": "Profile updated successfully!",
          "sortBy": "Sort by",
          "sortName": "Name",
          "sortTotalTasks": "Task Volume",
          "sortRate": "Success Rate",
          "userDetails": "User Performance Details",
          "err_email_required": "Email is required",
          "err_email_invalid": "Invalid email format",
          "err_password_required": "Password is required",
          "err_password_too_short": "Password must be at least 8 characters",
          "err_password_no_letter": "Password must contain at least one letter",
          "err_password_no_number": "Password must contain at least one number",
          "err_password_no_special": "Password must contain at least one special character (!@#$...)",
          "err_name_too_short": "Name is too short",
          "err_title_required": "Title is required",
          "err_title_short": "Title must be at least 3 characters",
          "err_title_long": "Title is too long (max 50)",
          "err_desc_required": "Description is required",
          "err_desc_short": "Description must be at least 10 characters",
          "err_status_invalid": "Invalid status value",
          "theme": "Theme",
          "logout": "Logout",
          "navigation": "Navigation",
          "resources": "Resources",
          "connect": "Connect",
          "allRightsReserved": "All rigths reserved.",
          "developedBy": "Develop by",
          "profile": "Profile",
          "terms": "Terms of Service",
          "policy": "Privacy Policy",
          "descriptionText": "Streamline your productivity, organize your day, and achieve your goals faster.",
          "projects": "Projects",
          "allProjects": "All Projects",
          "sortAlpha": "Sort A–Z",
          "sortRecent": "Sort by recent",
          "onlyMyTasks": "Only my tasks",
          "allTasks": "All tasks",
          "newProject": "New Project",
          "projectColor": "Project colour",
          "project": "Project",
          "noProject": "No project",
          "projectCreated": "Project created!",
          "projectCreateError": "Could not create project. Try again.",
          "create": "Create",
          "projectNamePlaceholder": "Project name...",
          "joinProject": "Join",
          "leaveProject": "Leave project",
          "projectOwnerLabel": "Owner",
          "projectMemberLabel": "Member",
          "projectOwnedBy": "Owner: {{name}}",
          "deleteProject": "Delete project",
          "deleteProjectWarning": "This will permanently delete all tasks in this project. This action cannot be undone.",
          "deleteProjectWarningTasks": "Tasks to be deleted:",
          "deleteProjectWarningMembers": "Members who will be notified:",
          "deleteProjectNotifyNote": "All members will receive an email notification.",
          "deleteProjectConfirm": "Delete project",
          "projectJoined": "You joined the project!",
          "projectJoinError": "Could not join the project. Try again.",
          "joinProjectConfirm": "Join project",
          "joinProjectWarning": "You are about to join this project and gain access to all its tasks.",
          "projectLeft": "You left the project.",
          "projectLeaveError": "Could not leave the project. Try again.",
          "leaveProjectConfirm": "Leave project",
          "leaveProjectWarning": "You will lose access to this project's tasks. You can rejoin at any time.",
          "projectDeleted": "Project deleted.",
          "projectDeleteError": "Could not delete the project. Try again.",
          "members": "members",
          "showLess": "Show less",
          "renameProject": "Rename project",
          "renameProjectPlaceholder": "New project name...",
          "projectRenamed": "Project renamed!",
          "projectRenameError": "Could not rename the project. Try again.",
          "save": "Save",
          "category": "Category",
          "noCategory": "No category",
          "err_categoryId_invalid": "Invalid category selected",
          "notYourTask": "You can only modify your own tasks",
          "manageProject": "Manage project",
          "publicProject": "Public",
          "privateProject": "Private",
          "visibilityToggle": "Visibility",
          "public": "Public",
          "private": "Private",
          "membersList": "Members",
          "addMember": "Add member",
          "addMemberPlaceholder": "User email...",
          "memberAdded": "Member added successfully",
          "memberAddError": "Could not add member. Check the email and try again.",
          "memberRemoved": "Member removed",
          "memberRemoveError": "Could not remove member. Try again.",
          "removeMember": "Remove member",
          "settingsUpdated": "Settings saved",
          "settingsUpdateError": "Could not save settings. Try again.",
          "tags": "Tags",
          "noTags": "No tags",
          "tagCreated": "Tag created!",
          "tagCreateError": "Could not create tag. Try again.",
          "tagDeleted": "Tag deleted.",
          "tagDeleteError": "Could not delete tag. Try again.",
          "tagAssigned": "Tag assigned!",
          "tagUnassigned": "Tag removed.",
          "assignTo": "Assign to",
          "assignedTo": "Assigned to",
          "noAssignees": "Unassigned",
          "onlyMyAssignments": "Assigned to me",
          "userAssigned": "User assigned!",
          "userUnassigned": "User unassigned.",
          "assignError": "Could not update the assignment. Try again.",
          "ownerAssignOnly": "Only the project owner can assign tasks.",
          "addTag": "Add tag",
          "deleteTag": "Delete tag",
          "tagNamePlaceholder": "Tag name...",
          "tagColor": "Tag color",
          "deleteTagWarning": "Delete tag",
          "deleteTagUsedIn": "It is used in",
          "task(s)": "task(s). This cannot be undone.",
          "removeMemberWarning": "will be removed from the project and lose access to all its tasks.",
          "priority": "Priority",
          "noPriority": "No priority",
          "dueDate": "Due date",
          "LOW": "Low",
          "MEDIUM": "Medium",
          "HIGH": "High",
          "URGENT": "Urgent",
          "err_priority_invalid": "Invalid priority value",
          "filterByCategory": "Filter by category",
          "filterByTag": "Filter by tag",
          "allTags": "All tags",
          "clearFilters": "Clear filters",
          "exportPdf": "Export PDF",
          "history": "History",
          "noHistory": "No history entries yet.",
          "historyAction_TASK_CREATED": "Task created",
          "historyAction_TASK_UPDATED": "Task updated",
          "historyAction_TASK_COMPLETED": "Task completed",
          "historyAction_TASK_DELETED": "Task deleted"
        }
      },
      es: {
        translation: {
          "appName": "GESTOR DE TAREAS",
          "home": "Inicio",
          "dashboard": "Tablero",
          "progress": "PROGRESO DEL TABLERO",
          "search": "Buscar tareas...",
          "createTask": "Crear Nueva Tarea",
          "title": "Título",
          "description": "Descripción",
          "addTask": "AÑADIR TAREA",
          "clear": "Limpiar",
          "noTasks": "No hay tareas aquí",
          "comments": "Comentarios",
          "noComments": "Aún no hay comentarios. ¡Sé el primero!",
          "commentPlaceholder": "Escribe un comentario...",
          "commentSend": "Enviar",
          "commentError": "No se pudo enviar el comentario. Inténtalo de nuevo.",
          "deleteComment": "Eliminar comentario",
          "deleteCommentConfirmTitle": "¿Eliminar este comentario?",
          "deleteCommentConfirmBody": "Esto eliminará el comentario de forma permanente para todos. Esta acción no se puede deshacer.",
          "deleteCommentError": "No se pudo eliminar el comentario. Inténtalo de nuevo.",
          "clearChat": "Vaciar chat",
          "clearChatConfirmTitle": "¿Vaciar el chat solo para ti?",
          "clearChatConfirmBody": "Esto ocultará todos los mensajes actuales solo en tu vista. Los demás participantes seguirán viéndolos. No se elimina nada del servidor.",
          "chatCleared": "Chat vaciado para ti.",
          "you": "Tú",
          "required": "requerido",
          "pending": "PENDIENTES",
          "inProgress": "EN CURSO",
          "completed": "COMPLETADAS",
          "all": "TODAS",
          "placeholderTitle": "¿Qué hay que hacer?",
          "placeholderDesc": "Añade algunos detalles...",
          "viewSource": "Ver código en GitHub",
          "syncing": "Sincronizando con la BD...",
          "editTask": "Editar Tarea",
          "taskDetails": "Detalles de la Tarea",
          "saveChanges": "Guardar Cambios",
          "cancel": "Cancelar",
          "close": "Cerrar",
          "deleteWarning": "¿Estás seguro de que deseas eliminar",
          "deleteAction": "Esta acción es permanente.",
          "deleteTask": "Eliminar Tarea",
          "moveTo": "Mover a",
          "noDescription": "Sin descripción",
          "noDetails": "No se proporcionaron detalles adicionales para esta tarea.",
          "sortNewest": "Más recientes",
          "sortOldest": "Más antiguas",
          "page": "Página",
          "of": "de",
          "prev": "Ant",
          "next": "Sig",
          "firstPage": "Primera página",
          "lastPage": "Última página",
          "kpiDashboard": "Panel de Indicadores",
          "totalTasks": "Total de Tareas",
          "completionRate": "Tasa de Finalización",
          "statusDistribution": "Distribución por Estados",
          "recentActivity": "Actividad del Sistema",
          "createdOn": "Creado",
          "updated": "Actualizado",
          "noActivity": "No hay actividad reciente.",
          "avgTime": "Tiempo Medio de Completado",
          "days": " días",
          "and": " y ",
          "hours": " horas",
          "lessThanDay": "<1 día",
          "boardHealth": "Salud del Tablero",
          "workloadTitle": "Carga de Trabajo por Estado",
          "healthExcellent": "EXCELENTE",
          "healthImprovable": "MEJORABLE",
          "errorTitle": "Problema de Conexión",
          "errorMessage": "No pudimos contactar con el servidor. Revisa tu conexión.",
          "errorLoadingTasks": "Error al sincronizar tareas",
          "retry": "Reintentar",
          "taskCreated": "¡Tarea creada con éxito!",
          "taskUpdated": "¡Tarea actualizada con éxito!",
          "taskDeleted": "Tarea eliminada",
          "clearCompleted": "Limpiar Completadas",
          "completedCleared": "Las tareas completadas han sido eliminadas",
          "clearCompletedWarning": "¿Estás seguro? Todas las tareas en estado COMPLETADO se borrarán permanentemente.",
          "requiredFieldsError": "El título y la descripción son obligatorios",
          "loginSubtitle": "Inicia sesión para gestionar tus tareas",
          "loginEmail": "Correo electrónico",
          "loginEmailPlaceholder": "tu@ejemplo.com",
          "loginPassword": "Contraseña", // NOSONAR: translation key, not a hardcoded credential
          "loginPasswordPlaceholder": "Tu contraseña",
          "loginButton": "Iniciar Sesión",
          "loginRequiredFields": "El correo y la contraseña son obligatorios",
          "loginError": "Credenciales incorrectas. Inténtalo de nuevo.",
          "accountBlocked": "Tu cuenta ha sido bloqueada. Contacta con un administrador.",
          "togglePassword": "Mostrar/ocultar contraseña",
          "adminPanel": "Panel de Administrador",
          "adminRole": "Administrador", 
          "userRole": "Usuario",
          "adminTotalUsers": "Total de Usuarios",
          "adminTotalTasks": "Total de Tareas",
          "adminInProgress": "En Curso",
          "adminTasksPerUser": "Tareas por Usuario",
          "resourceManagement": "Gestión de Carga",
          "leadTimeByCategory": "Tiempo Medio de Resolución por Categoría",
          "avgLeadTime": "Tiempo Medio",
          "overloaded": "Sobrecargado",
          "healthy": "Saludable",
          "noActiveWork": "No hay tareas activas en este momento.",
          "noLeadTimeData": "Aún no hay tareas completadas para calcular el tiempo.",
          "rangeLast7": "Últimos 7 días",
          "rangeLast30": "Últimos 30 días",
          "rangeLast90": "Últimos 90 días",
          "rangeAll": "Todo el tiempo",
          "adminUserList": "Gestión de Usuarios",
          "adminSearchUsers": "Buscar por nombre o email...",
          "adminColName": "Nombre",
          "adminColEmail": "Correo",
          "adminColRole": "Rol",
          "adminColTotal": "Total",
          "adminColActions": "Acciones",
          "adminPromote": "Promover",
          "adminDemote": "Degradar",
          "adminPromoted": "Usuario promovido a Admin",
          "adminDemoted": "Usuario degradado a User",
          "adminPromoteConfirm": "¿Promover a {{name}} a ADMIN? Tendrá acceso completo.",
          "adminDemoteConfirm": "¿Degradar a {{name}} a USER? Perderá el acceso de admin.",
          "blockUser": "Bloquear",
          "unblockUser": "Desbloquear",
          "blockUserConfirm": "¿Bloquear a {{name}}? Será expulsado en su próxima petición.",
          "unblockUserConfirm": "¿Desbloquear a {{name}}? Recuperará el acceso de inmediato.",
          "userBlocked": "Usuario bloqueado correctamente.",
          "userUnblocked": "Usuario desbloqueado correctamente.",
          "deleteUser": "Eliminar usuario",
          "userDeleted": "Usuario eliminado.",
          "deleteUserConfirm": "¿Eliminar a {{name}}?",
          "deleteUserWarning": "Esto eliminará permanentemente la cuenta y todas sus tareas ({{count}}). Esta acción no se puede deshacer.",
          "noResults": "No se encontraron resultados.",
          "registerSubtitle": "Crea tu cuenta",
          "registerName": "Nombre (opcional)",
          "registerNamePlaceholder": "Tu nombre",
          "registerConfirmPassword": "Confirmar Contraseña",
          "registerConfirmPasswordPlaceholder": "Repite tu contraseña",
          "registerButton": "Crear Cuenta",
          "registerLink": "¿No tienes cuenta? Regístrate",
          "loginLink": "¿Ya tienes cuenta? Inicia sesión",
          "registerPasswordsNoMatch": "Las contraseñas no coinciden.",
          "registerPasswordTooShort": "La contraseña debe tener al menos 6 caracteres.",
          "registerEmailTaken": "Este correo ya está registrado.",
          "editProfileTitle": "Editar Perfil",
          "editProfileName": "Nombre a mostrar",
          "editProfileSave": "Guardar",
          "editProfileSuccess": "¡Perfil actualizado correctamente!",
          "sortBy": "Ordenar por",
          "sortName": "Nombre",
          "sortTotalTasks": "Volumen de Tareas",
          "sortRate": "Tasa de Éxito",
          "userDetails": "Detalles de Rendimiento",
          "err_email_required": "El correo electrónico es obligatorio",
          "err_email_invalid": "El formato del correo no es válido",
          "err_password_required": "La contraseña es obligatoria",
          "err_password_too_short": "La contraseña debe tener al menos 8 caracteres",
          "err_password_no_letter": "La contraseña debe contener al menos una letra",
          "err_password_no_number": "La contraseña debe contener al menos un número",
          "err_password_no_special": "La contraseña debe contener al menos un carácter especial (!@#$...)",
          "err_name_too_short": "El nombre es demasiado corto",
          "err_title_required": "El título es obligatorio",
          "err_title_short": "El título debe tener al menos 3 caracteres",
          "err_title_long": "El título es demasiado largo (máx. 50)",
          "err_desc_required": "La descripción es obligatoria",
          "err_desc_short": "La descripción debe tener al menos 10 caracteres",
          "err_status_invalid": "Estado no válido",
          "theme": "Tema",
          "logout": "Cerrar Sesión",
          "navigation": "Navegación",
          "resources": "Recursos",
          "connect": "Conectar",
          "allRightsReserved": "Todos los derechos reservados.",
          "developedBy": "Desarrollado por",
          "profile": "Perfil",
          "terms": "Terminos de servicio",
          "policy": "Política de Privacidad",
          "descriptionText": "Optimiza tu productividad, organiza tu día y alcanza tus objetivos más rápido.",
          "projects": "Proyectos",
          "allProjects": "Todos los proyectos",
          "sortAlpha": "Ordenar A–Z",
          "sortRecent": "Ordenar por reciente",
          "onlyMyTasks": "Solo mis tareas",
          "allTasks": "Todas las tareas",
          "newProject": "Nuevo Proyecto",
          "projectColor": "Color del proyecto",
          "project": "Proyecto",
          "noProject": "Sin proyecto",
          "projectCreated": "¡Proyecto creado!",
          "projectCreateError": "No se pudo crear el proyecto. Inténtalo de nuevo.",
          "create": "Crear",
          "projectNamePlaceholder": "Nombre del proyecto...",
          "joinProject": "Unirse",
          "leaveProject": "Salir del proyecto",
          "projectOwnerLabel": "Propietario",
          "projectMemberLabel": "Miembro",
          "projectOwnedBy": "Propietario: {{name}}",
          "deleteProject": "Eliminar proyecto",
          "deleteProjectWarning": "Esto eliminará permanentemente todas las tareas de este proyecto. Esta acción no se puede deshacer.",
          "deleteProjectWarningTasks": "Tareas que se eliminarán:",
          "deleteProjectWarningMembers": "Miembros que serán notificados:",
          "deleteProjectNotifyNote": "Todos los miembros recibirán una notificación por email.",
          "deleteProjectConfirm": "Eliminar proyecto",
          "projectJoined": "¡Te uniste al proyecto!",
          "projectJoinError": "No se pudo unir al proyecto. Inténtalo de nuevo.",
          "joinProjectConfirm": "Unirse al proyecto",
          "joinProjectWarning": "Estás a punto de unirte a este proyecto y tendrás acceso a todas sus tareas.",
          "projectLeft": "Saliste del proyecto.",
          "projectLeaveError": "No se pudo salir del proyecto. Inténtalo de nuevo.",
          "leaveProjectConfirm": "Salir del proyecto",
          "leaveProjectWarning": "Perderás acceso a las tareas de este proyecto. Puedes volver a unirte más tarde.",
          "projectDeleted": "Proyecto eliminado.",
          "projectDeleteError": "No se pudo eliminar el proyecto. Inténtalo de nuevo.",
          "members": "miembros",
          "showLess": "Ver menos",
          "renameProject": "Renombrar proyecto",
          "renameProjectPlaceholder": "Nuevo nombre del proyecto...",
          "projectRenamed": "¡Proyecto renombrado!",
          "projectRenameError": "No se pudo renombrar el proyecto. Inténtalo de nuevo.",
          "save": "Guardar",
          "category": "Categoría",
          "noCategory": "Sin categoría",
          "err_categoryId_invalid": "Categoría seleccionada no válida",
          "notYourTask": "Solo puedes modificar tus propias tareas",
          "manageProject": "Gestionar proyecto",
          "publicProject": "Público",
          "privateProject": "Privado",
          "visibilityToggle": "Visibilidad",
          "public": "Público",
          "private": "Privado",
          "membersList": "Miembros",
          "addMember": "Añadir miembro",
          "addMemberPlaceholder": "Email del usuario...",
          "memberAdded": "Miembro añadido correctamente",
          "memberAddError": "No se pudo añadir al miembro. Verifica el email e inténtalo de nuevo.",
          "memberRemoved": "Miembro eliminado",
          "memberRemoveError": "No se pudo eliminar al miembro. Inténtalo de nuevo.",
          "removeMember": "Eliminar miembro",
          "settingsUpdated": "Configuración guardada",
          "settingsUpdateError": "No se pudo guardar la configuración. Inténtalo de nuevo.",
          "tags": "Etiquetas",
          "noTags": "Sin etiquetas",
          "tagCreated": "¡Etiqueta creada!",
          "tagCreateError": "No se pudo crear la etiqueta. Inténtalo de nuevo.",
          "tagDeleted": "Etiqueta eliminada.",
          "tagDeleteError": "No se pudo eliminar la etiqueta. Inténtalo de nuevo.",
          "tagAssigned": "¡Etiqueta asignada!",
          "tagUnassigned": "Etiqueta eliminada.",
          "assignTo": "Asignar a",
          "assignedTo": "Asignada a",
          "noAssignees": "Sin asignar",
          "onlyMyAssignments": "Asignadas a mí",
          "userAssigned": "¡Usuario asignado!",
          "userUnassigned": "Usuario desasignado.",
          "assignError": "No se pudo actualizar la asignación. Inténtalo de nuevo.",
          "ownerAssignOnly": "Solo el propietario del proyecto puede asignar tareas.",
          "addTag": "Añadir etiqueta",
          "deleteTag": "Eliminar etiqueta",
          "tagNamePlaceholder": "Nombre de la etiqueta...",
          "tagColor": "Color de la etiqueta",
          "deleteTagWarning": "Eliminar etiqueta",
          "deleteTagUsedIn": "Está siendo usada en",
          "task(s)": "tarea(s). Esta acción no se puede deshacer.",
          "removeMemberWarning": "será eliminado del proyecto y perderá acceso a todas sus tareas.",
          "priority": "Prioridad",
          "noPriority": "Sin prioridad",
          "dueDate": "Fecha límite",
          "LOW": "Baja",
          "MEDIUM": "Media",
          "HIGH": "Alta",
          "URGENT": "Urgente",
          "err_priority_invalid": "Valor de prioridad no válido",
          "filterByCategory": "Filtrar por categoría",
          "filterByTag": "Filtrar por etiqueta",
          "allTags": "Todas las etiquetas",
          "clearFilters": "Limpiar filtros",
          "exportPdf": "Exportar PDF",
          "history": "Historial",
          "noHistory": "Sin entradas de historial todavía.",
          "historyAction_TASK_CREATED": "Tarea creada",
          "historyAction_TASK_UPDATED": "Tarea actualizada",
          "historyAction_TASK_COMPLETED": "Tarea completada",
          "historyAction_TASK_DELETED": "Tarea eliminada"
        }
      }
    },
    fallbackLng: "en",
    interpolation: { escapeValue: false }
  });

export default i18n;



